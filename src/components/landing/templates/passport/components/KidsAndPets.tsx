...
export function KidsAndPets({ acceptsKids, acceptsPets, className = '' }: KidsAndPetsProps) {
  ...
  const content = (
    <div className={`grid ${acceptsKids && acceptsPets ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'} gap-8`}>
      {acceptsKids && (
        <motion.div 
          className="bg-[#C5A467] rounded-xl p-8 text-center border border-[#CFD6BA]/20"
          variants={item}
        ...
        >
          ...
          >
            <Baby className="w-12 h-12 text-[#4C1C1D]" />
          </motion.div>
          <h3 className="text-2xl font-sans mb-4 text-[#4C1C1D]">Los Niños Bienvenidos</h3>
          <p className="text-[#4C1C1D]/80 text-lg leading-relaxed font-sans">
      ...
      {acceptsPets && (
        ...
        >
          <motion.div 
            ...
            whileHover={{ rotate: 15 }}
          >
            <PawPrint className="w-12 h-12 text-[#4C1C1D]" />
          </motion.div>
          <h3 className="text-2xl font-sans mb-4 text-[#4C1C1D]">Pet Friendly</h3>
          <p className="text-[#4C1C1D]/80 text-lg leading-relaxed font-sans">
