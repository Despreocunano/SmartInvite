...
export function KidsAndPets({ acceptsKids, acceptsPets, className = '' }: KidsAndPetsProps) {
  ...
  const content = (
    <div className={`grid ${acceptsKids && acceptsPets ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'} gap-8`}>
      {acceptsKids && (
        <motion.div 
          className="bg-[#869484] rounded-xl p-8 text-center border border-[#CFD6BA]/20"
          variants={item}
  ...
  return (
    <section className={`px-4 ${className}`}>
      <motion.div 
        className="bg-[#F5F7F2] w-full max-w-4xl mx-auto flex flex-col items-center justify-center px-8 py-16"
        initial="hidden"
        whileInView="show"
        viewport={{ once: true }}
        variants={container}
