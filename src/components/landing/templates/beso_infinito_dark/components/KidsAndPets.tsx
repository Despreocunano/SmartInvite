...
export function KidsAndPets({ acceptsKids, acceptsPets, className = '' }: KidsAndPetsProps) {
  ...
  const content = (
    <div className={`grid ${acceptsKids && acceptsPets ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'} gap-8`}>
      {acceptsKids && (
        <motion.div 
          className="bg-[#31363F] rounded-xl p-8 text-center border border-[#CFD6BA]/20"
          variants={item}
        ...
        >
          ...
          >
            <Baby className="w-14 h-14 text-[#77ABAE]" />
          </motion.div>
          <h3 className="text-3xl font-sans mb-4 text-[#EEEEEE]">Los Niños Bienvenidos</h3>
          <p className="text-[#EEEEEE]/80 text-lg leading-relaxed font-sans">
      ...
      {acceptsPets && (
        ...
        >
          <motion.div 
            ...
            whileHover={{ rotate: 15 }}
          >
            <PawPrint className="w-14 h-14 text-[#77ABAE]" />
          </motion.div>
          <h3 className="text-3xl mb-4 text-[#EEEEEE] font-sans">Pet Friendly</h3>
          <p className="text-[#EEEEEE]/80 text-lg leading-relaxed font-sans">
    ...
    <section className={`p-24 px-4 ${className}`}>
      ...
      >
        <motion.div className="text-center mb-12" variants={item}>
          <h2 className="text-5xl md:text-6xl font-fraunces text-[#EEEEEE] mb-2">
